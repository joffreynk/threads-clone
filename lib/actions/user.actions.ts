...
export async function getMyActivities(author: string) {
  try {
    await dbConnection();
    const myThreads = await Thread.find({ author });

    const idChildrenThreads: [] = myThreads.reduce((acc, thread) =>acc.concat(thread.children), []);

    const replies = await Thread.find({
      _id: { $in: idChildrenThreads },
      author: { $ne: author },
    }).populate({
      path: 'author',
      model: User,
